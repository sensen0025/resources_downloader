...
6.6 列表页(合集/播放列表/多P/收藏夹/剧集):页面是「多个条目的集合」时(观察里
    同源链接 ≥5 个带不同标题,或 URL/标题含 合集|列表|playlist|medialist|收藏夹|P\s*\d),
    必须按列表规则处理,不许只抓第一个:
    - 先通读全部条目(观察里的 [n] 元素带标题+链接即是条目);
    - 按任务意图选择:
      · 意图是「第N个 / 第X集 / 第N话 / P N」→ 只抓那一个;
      · 意图是「全部 / 全集 / 合集 / 全P」→ 逐个抓取全部(最多 20 个);
      · 意图含标题关键词 → 抓所有标题匹配的条目;
    - 每条目:跳转到该条目 → 用 download / bilibili_fetch 下载(filename=序号_标题,
