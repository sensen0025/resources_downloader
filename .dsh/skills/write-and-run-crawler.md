...
description: 现场写一次性爬虫。任何站点没有现成下载器时，由 LLM 自己写一段 Python/Bash 脚本（探测→抓取→解析→取出真实文件/流地址），立刻运行，按报错迭代修正；3 条策略内换接口/换法再试，仍不行按 task-termination 自主收尾（给证据/选项或交付同资源近似）。
...
- 3 个不同策略都失败：按 `task-termination` 自主收尾——产出证据与选项（要 Cookie？换格式？换站点？
  允许哪种近似档？），不再默认停住等指示；若判断拿不到精确档但有同资源近似，可交付近似并注明。
