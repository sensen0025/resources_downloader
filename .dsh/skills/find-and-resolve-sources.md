...
- 找不到精确规格时，注意同一资源可能以相邻档/不同格式存在（如低一档清晰度、zip 版）——
  记下这类「近似候选」，供 resource-download / task-termination 决定是否接受近似交付。
...
```
找不到任何可下载源时，按 `task-termination` 收尾：说清楚试过哪些关键词/站点、失败原因与状态码、
下一步建议（换关键词/换站/提供直链或登录态/允许哪种近似档），不要编造 URL，也不要无限再搜。
